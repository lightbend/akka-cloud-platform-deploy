...
export class Backends {
  install(provider: k8s.Provider): void {
    new Grafana().install(provider);
    new Prometheus().install(provider);
  }
}

class Prometheus {
  install(k8sProvider: k8s.Provider): void {
...
class Grafana {
  ...
  install(k8sProvider: k8s.Provider): Promise<void> {
    return this.getDashboards()
      ...
      .catch((reason) => {
        ...
        this.simpleInstall(k8sProvider);
      });
  }
